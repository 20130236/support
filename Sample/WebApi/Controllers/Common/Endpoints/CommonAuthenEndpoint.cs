        ...
        where TEntity : class, IStrongEntity
    {
        ...
        public IActionResult Login([FromBody] AccountLoginDto input)
        {
            ...
            else
            {
                ...
                var saveToken = Service.CreateWithMapper(new AccountRefreshTokenAdminDto()
                {
                    Token = refreshToken.Result
                });
                //return DataResult(result.Id);

                return DataResult(new AccountDataAdminDto()
                {
        ...
        [Transactional]
        public IActionResult RefreshToken([FromBody] AccountRefreshTokenAdminDto input)
        {
            if (AccountService.AvailableRefreshToken(input.Token))
            {
            ...
            var errorResult = new ErrorResult(400, "sigup_failed", "Token bị sai hoặc không tồn tại");
            return errorResult;
        }
        ...
        private async Task<string> GenerateAccessToken(Account account)
        {
            ...
            var tokenDescription = new SecurityTokenDescriptor
            {
                Subject = new ClaimsIdentity(new[] {
                    ...
                    new Claim("iss", "IDENTITY"),
                    new Claim("scope", account.RoleCode.ToString())
                }),
